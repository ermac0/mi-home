...
namespace MiLight
{
	...
	public class MiLightGateway
	{
		...
		public MiLightGateway(IPAddress ip) : this(ip.ToString())
		{
		}
		...
		/// </summary>
		/// <param name="ip">The IP-address of the MiLight WiFi controller.</param>
		public MiLightGateway(string ip)
		{
		...
		public static List<MiLightGateway> Discover()
		{
			...
			var controllers = new List<MiLightGateway>();
			var ips = ep.ToString().Split(':');
			for (int i = 0; i < ips.Length; i+=2) {
				controllers.Add(new MiLightGateway(ips[i]));
			}
